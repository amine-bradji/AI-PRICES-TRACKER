...
import type { ComputedOffer as PublicOffer } from "@/lib/types";
  ...
  for (const region of REGIONS) {
    ...
    for (const provider of PROVIDERS) {
      for (const tier of provider.tiers) {
        ...
        // optionally this tier)?
        const override = REGIONAL_OFFERS.find(
          (o) =>
            o.providerId === provider.id &&
            o.regionCode === region.code &&
            (o.tierId == null || o.tierId === tier.id),
        );
